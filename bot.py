...
async def init_db():
    global db_pool
    if not DATABASE_URL:
        logger.warning("No DATABASE_URL set — reminders will not persist")
        return
    try:
...
async def save_reminder(chat_id, text, due):
    if not db_pool:
        return
    try:
        async with db_pool.acquire() as conn:
            await conn.execute(
                "INSERT INTO reminders (chat_id, text, due) VALUES ($1, $2, $3)",
                chat_id, text, due
            )
    except Exception as e:
    ...
    try:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM reminders WHERE fired = FALSE AND due > NOW() ORDER BY due"
            )
        return rows
    except Exception as e:
...
async def mark_reminder_fired(reminder_id):
    if not db_pool:
        return
    try:
        async with db_pool.acquire() as conn:
            await conn.execute("UPDATE reminders SET fired = TRUE WHERE id = $1", reminder_id)
    except Exception as e:
        logger.error(f"Mark fired error: {e}")
    ...
    while True:
        try:
            if db_pool:
                async with db_pool.acquire() as conn:
                    rows = await conn.fetch(
                        "SELECT * FROM reminders WHERE fired = FALSE AND due <= NOW()"
                    )
                for row in rows:
                    try:
                        ...
                        logger.info(f"Fired reminder: {row['text']}")
                    except Exception as e:
                        logger.error(f"Fire reminder error: {e}")
        except Exception as e:
...
async def remind_cmd(update, context):
    ...
    try:
        ...
        return
    await save_reminder(update.effective_chat.id, reminder_text, due)
...
async def draft_cmd(update, context):
    ...
    if not brief:
        await update.message.reply_text("Usage: /draft [type] [brief]\n\nExample: /draft email follow up with NHS contact re TapVitals pilot")
        return
...
async def brain_cmd(update, context):
    ...
    if rows:
        brain += f"UPCOMING REMINDERS ({len(rows)}):\n"
        for r in rows:
...
async def weekly_cmd(update, context):
    ...
    reminders_summary = "\n".join([f"- {r['text']}" for r in rows[:5]]) if rows else "None"
    prompt = f"""Generate a weekly review for Lamar. Today is {now.strftime('%A %d %B %Y')}.

Recent ideas: {ideas_summary}
Upcoming reminders: {reminders_summary}
...
async def price_cmd(update, context):
    ...
    if not brief:
        ...
        return
    await update.message.chat.send_action("typing")
    prompt = f"Do a pricing sanity check for Lamar. Consider market, perceived value, competition, positioning. Be direct.\n\nProduct/context: {brief}\n\nGive: recommended price point, reasoning, and any red flags."
    reply = await ask_claude(prompt)
    await update.message.reply_text(reply)
